...
import { Close as CloseIcon, CheckBox as CheckBoxIcon, Add as AddIcon } from '@mui/icons-material';
...
export const ChecklistModal: React.FC<ChecklistModalProps> = ({
  ...
  const handleToggleComplete = useCallback(
    async (itemId: string, isComplete: boolean) => {
      ...
      setChecklistItems((prevItems) =>
        prevItems.map((item) =>
          item.checklistItemId === itemId ? { ...item, isComplete: !isComplete } : item,
        ),
      );
      ...
      try {
        ...
        setChecklistItems((prevItems) =>
          prevItems.map((item) => (item.checklistItemId === itemId ? updatedItem : item)),
        );
      } catch (error) {
        ...
        setFetchError(
          i18n._('checklist.error.update', { default: 'Failed to update item. Please try again.' }),
        );
      }
    },
  ...
  const handleDeleteItem = useCallback(
    async (itemId: string) => {
      ...
      try {
        ...
        setFetchError(
          i18n._('checklist.error.delete', { default: 'Failed to delete checklist item' }),
        );
      }
    },
